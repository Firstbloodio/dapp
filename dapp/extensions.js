...
const createPopup = (file) => {
  ...
  metaWindow.on("close", () => {
    ...
    if(allWindows.length > 0){
      allWindows[0].webContents.send( 'close-metamask' );      
    }
  });
...
const loadMetamask = (session, window, isDev) => {
  ...
  ipcMain.on('open-metamask-notification-2', (event, arg) => {
    //if(metamaskNotification && !metamaskNotification.isDestroyed()) metamaskNotification.close();

    //metamaskNotification = createPopup();
    metamaskPopup.loadURL(`chrome-extension://${METAMASK_ID}/notification.html`);
  });
